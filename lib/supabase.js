...
// Permanently deletes a client and all data linked by client_id (brand intake,
// portal projects, marketing dashboards, campaigns, brand hubs, notes).
// This is destructive and irreversible. The dashboard requires the team to
// type DELETE before this runs.
export async function deleteClient_db(id) {
  if (!supabase) return false
  // Best-effort cleanup of linked rows. Errors are logged but don't block deletion.
  const tables = [
    'brand_intake',
    'brand_hubs',
    'portal_projects',
  ...
  for (const t of tables) {
    const { error } = await supabase.from(t).delete().eq('client_id', id)
    if (error) console.warn(`Delete ${t} for client ${id}:`, error.message)
  }
