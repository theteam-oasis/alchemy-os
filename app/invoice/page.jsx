...
function generateInvoicePDF(invoice) {
  ...
  const hasWire = invoice.wireBank || invoice.wireRouting || invoice.wireAccount;
  const hasZelle = invoice.zellePhone;

  if (hasWire || hasZelle || invoice.paymentTerms) {
    // Calculate box height first
    let boxH = 16 + 10; // padding top + header
    if (invoice.paymentTerms) boxH += 18;
    if (hasWire) {
      boxH += 14; // "Wire Transfer" heading
      [invoice.wireBank, invoice.wireBankAddress, invoice.wireRouting, invoice.wireAccount, invoice.wireSwift, invoice.wireName]
        .forEach(v => { if (v) boxH += 13; });
      boxH += 8;
    }
    if (hasZelle) boxH += 14 + 13 + 4;
    boxH += 10; // padding bottom
    ...
    doc.setFont("helvetica", "bold");
    doc.setTextColor(134, 134, 139);
    doc.text("PAYMENT INSTRUCTIONS", margin + 14, ry);
    ry += 16;
    ...
    if (invoice.paymentTerms) {
      ...
      doc.setFont("helvetica", "bold");
      doc.setTextColor(29, 29, 31);
      doc.text("Terms:", margin + 14, ry);
      doc.setFont("helvetica", "normal");
      doc.setTextColor(80, 80, 85);
      doc.text(invoice.paymentTerms, margin + 14 + doc.getTextWidth("Terms:  "), ry);
      ry += 18;
    }

    const kvLine = (label, val) => {
      if (!val) return;
      doc.setFontSize(9.5);
      doc.setFont("helvetica", "bold");
      doc.setTextColor(134, 134, 139);
      ...
      doc.setFont("helvetica", "normal");
      doc.setTextColor(29, 29, 31);
      doc.text(String(val), margin + 130, ry);
      ry += 13;
    };
    ...
    if (hasWire) {
      ...
      doc.setFont("helvetica", "bold");
      doc.setTextColor(29, 29, 31);
      doc.text("Wire Transfer", margin + 14, ry);
      ry += 14;
      kvLine("Bank", invoice.wireBank);
      kvLine("Bank Address", invoice.wireBankAddress);
      kvLine("Routing Number", invoice.wireRouting);
      kvLine("Account Number", invoice.wireAccount);
      kvLine("SWIFT", invoice.wireSwift);
      kvLine("Account Name", invoice.wireName);
      ry += 6;
    }
    ...
    if (hasZelle) {
      ...
      doc.setFont("helvetica", "bold");
      doc.setTextColor(29, 29, 31);
      doc.text("Zelle", margin + 14, ry);
      ry += 14;
      kvLine("Phone", invoice.zellePhone);
    }
  ...
  doc.text("Generated by ALCHEMY Productions", pw / 2, ph - 30, { align: "center" });
...
export default function InvoicePage() {
  const [fromName, setFromName] = useState("ALCHEMY Productions");
  ...
  const [paymentTerms, setPaymentTerms] = useState("Due on invoice receipt");

  const [items, setItems] = useState([{ description: "", quantity: "1", rate: "" }]);
  const [taxRate, setTaxRate] = useState("0");
  const [discount, setDiscount] = useState("0");
  const [notes, setNotes] = useState("Payment is due on receipt of this invoice. Thank you for your business.");

  // ── Payment Instructions ──
  const [wireBank, setWireBank] = useState("Bank of America");
  const [wireBankAddress, setWireBankAddress] = useState("222 Broadway, New York, NY 10038");
  const [wireRouting, setWireRouting] = useState("026009593");
  const [wireAccount, setWireAccount] = useState("898115266103");
  const [wireSwift, setWireSwift] = useState("BOFAUS3NXXX");
  const [wireName, setWireName] = useState("Dunbar Enterprises");
  const [zellePhone, setZellePhone] = useState("929-452-7048");
  ...
  const handleExport = () => {
    generateInvoicePDF({
      invoiceNumber, date, dueDate, paymentTerms,
      ...
      wireBank, wireBankAddress, wireRouting, wireAccount, wireSwift, wireName,
      zellePhone,
    });
  };
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          ...
          <Section title="Payment Instructions">
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              <div>
                <div style={{ fontSize: 13, fontWeight: 600, color: C.text, marginBottom: 10 }}>Wire Transfer</div>
                <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
                  <Input label="Bank" value={wireBank} onChange={setWireBank} half />
                  <Input label="Account Name" value={wireName} onChange={setWireName} half />
                  <Input label="Bank Address" value={wireBankAddress} onChange={setWireBankAddress} />
                  <Input label="Routing Number" value={wireRouting} onChange={setWireRouting} half />
                  <Input label="Account Number" value={wireAccount} onChange={setWireAccount} half />
                  <Input label="SWIFT" value={wireSwift} onChange={setWireSwift} half />
                </div>
              </div>
              <div style={{ paddingTop: 12, borderTop: `1px solid ${C.borderLight}` }}>
                <div style={{ fontSize: 13, fontWeight: 600, color: C.text, marginBottom: 10 }}>Zelle</div>
                <Input label="Phone" value={zellePhone} onChange={setZellePhone} placeholder="929-452-7048" />
              </div>
            </div>
          </Section>
