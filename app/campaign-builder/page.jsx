...
// ─── Main Component ───
export default function CampaignBuilder({ searchParams }) {
  ...
  const [scenes, setScenes] = useState([]) // [{options:[url,url,url,url], chosen: idx}]
  ...
  async function handleGenerateAvatars() {
    ...
    try {
      // Get avatar prompts from Claude
      ...
      const imageResults = await Promise.allSettled(
        prompts.map(p => generateImage(p.imagePrompt))
      )
  ...
  async function handleGenerateScenes() {
    ...
    try {
      // Generate all scenes — for each scene get 4 image options
      const allScenes = []
      for (let i = 0; i < sceneCount; i++) {
        // Get scene prompts from Claude
        ...
        // Generate 4 images for this scene in parallel
        const imageResults = await Promise.allSettled(
          json.scenePrompts.map(p => generateImage(p))
        )
        const images = imageResults.map(r => r.status === 'fulfilled' ? r.value : null)
        allScenes.push({ options: images, chosen: null })

        // Update scenes progressively as each one completes
        setScenes(prev => {
          const updated = [...prev]
    ...
    <>
      <style>{`
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .dot-item.done .dot {
          background: #FFD60A44;
          border-color: #FFD60A88;
        }
        .dot-item.active .dot {
          background: #FFD60A;
          border-color: #FFD60A;
          box-shadow: 0 0 12px #FFD60A66;
        }
      ...
      <div className="cb-shell">
        ...
        <header className="cb-header">
          ...
          {step > STEPS.INPUT && (
            <button className="cb-back-btn" onClick={() => setStep(s => Math.max(s - 1, STEPS.INPUT))}>
              ← Back
            </button>
          )}
        </header>
        ...
        <main className="cb-content">
          ...
          {step === STEPS.INPUT && (
            <div>
              <div className="step-heading">
                ...
                <p className="step-sub">Connect a client or enter brand details manually. The system will analyze the website before generating concepts.</p>
              </div>
              ...
              {clients.length > 0 && (
                <div className="client-selector">
                  ...
                  <div className="client-pills">
                    <button
                      className={`client-pill ${!selectedClientId ? 'active' : ''}`}
                      onClick={() => { setSelectedClientId(null); setBrandIntake(null) }}
                    >
                      No client
                    </button>
                    {clients.map(c => (
              ...
              <div className="form-grid">
                ...
                <div className="form-group full">
                  ...
                  <textarea
                    ...
                    placeholder="Any specific offer, launch, or context for this campaign..."
                    value={offerNotes}
                ...
                <div className="form-group full">
                  ...
                  <input
                    className="form-input"
                    placeholder="ritual, cinematic, rebellion, luxury, transformation..."
                    value={creativeKeywords}
                    onChange={e => setCreativeKeywords(e.target.value)}
                  />
                  <p className="keywords-hint">Comma-separated words to steer creative direction without writing prompts</p>
                </div>
                <div className="form-group">
                  ...
                  <div className="count-toggle">
                    {[4, 6].map(n => (
                      <button
                        key={n}
                        className={`count-btn ${conceptCount === n ? 'active' : ''}`}
          ...
          {step === STEPS.ANALYZING && (
            <LoadingPulse message="Reading the brand. Building intelligence..." />
          )}
            ...
            <div>
              <div className="step-heading">
                ...
                <p className="step-sub">
                  {conceptsLoading
                    ? 'Generating website-grounded concepts...'
                    : `${concepts.length} directions generated. Choose one to develop.`}
                </p>
              </div>
              ...
              {conceptsLoading ? (
                <LoadingPulse message={`Generating ${conceptCount} distinct creative directions...`} />
              ) : (
                <>
                  <div className="cards-grid" style={{ gridTemplateColumns: conceptCount === 6 ? 'repeat(3, 1fr)' : 'repeat(2, 1fr)' }}>
                    {concepts.map((c, i) => (
                      <ConceptCard
                        key={i}
                        concept={c}
                        selected={chosenConcept?.title === c.title}
                        onClick={() => setChosenConcept(c)}
                      />
                    ))}
                  </div>
                  ...
                  <div className="action-row">
                    <button
                      ...
                      disabled={!chosenConcept}
                      onClick={handleGenerateScripts}
                    >
                      Develop This Concept →
                    </button>
                    <button className="btn-secondary" onClick={() => handleGenerateConcepts()}>
                      Regenerate All
                    </button>
                  </div>
                </>
          ...
          {step === STEPS.SCRIPTS && (
            <div>
              <div className="step-heading">
                ...
                <p className="step-sub">
                  {scriptsLoading ? 'Writing real ad scripts...' : 'Four distinct scripts for your chosen concept. Pick the one that lands.'}
                </p>
              </div>
                ...
                <div className="form-group" style={{ marginBottom: 24 }}>
                  ...
                  <div className="duration-options">
                    {[15, 30, 45, 60].map(d => (
                      <button
                        key={d}
                        className={`duration-btn ${scriptDuration === d ? 'active' : ''}`}
              ...
              {scriptsLoading ? (
                <LoadingPulse message="Writing scripts for your chosen concept..." />
              ) : (
                <>
                  <div className="cards-grid">
                    {scripts.map((s, i) => (
                      <ScriptCard
                        key={i}
                        script={s}
                        selected={chosenScript?.title === s.title}
                        onClick={() => setChosenScript(s)}
                      />
                    ))}
                  </div>
                  ...
                  <div className="action-row">
                    <button
                      className="btn-primary"
                      disabled={!chosenScript}
                      onClick={handleGenerateDirections}
                    >
                      Choose Visual Direction →
                    </button>
                    <button className="btn-secondary" onClick={handleGenerateScripts}>
                      Regenerate Scripts
                    </button>
                  </div>
                </>
          ...
          {step === STEPS.DIRECTIONS && (
            <div>
              <div className="step-heading">
                ...
                <p className="step-sub">
                  {directionsLoading ? 'Generating visual worlds...' : 'Four distinct cinematographic directions. Choose the world your campaign lives in.'}
                </p>
              </div>
              ...
              {directionsLoading ? (
                <LoadingPulse message="Building four distinct visual worlds..." />
              ) : (
                <>
                  <div className="cards-grid">
                    {directions.map((d, i) => (
                      <DirectionCard
                        key={i}
                        direction={d}
                        selected={chosenDirection?.title === d.title}
                  ...
                  <div className="action-row">
                    <button
                      className="btn-primary"
                      disabled={!chosenDirection}
                      onClick={handleGenerateAvatars}
                    >
                      Cast the Avatar →
                    </button>
                    <button className="btn-secondary" onClick={handleGenerateDirections}>
          ...
          {step === STEPS.AVATAR && (
            <div>
              <div className="step-heading">
                ...
                <p className="step-sub">
                  {avatarsLoading
                    ? 'Generating four avatar candidates...'
                    : 'Choose one. This character will appear consistently across every scene.'}
                </p>
              </div>
              ...
              <div className="image-grid">
                {avatarImages.map((img, i) => (
                  <div key={i} className="avatar-item">
                    <div
                      className={`image-slot ${!avatarsLoading && chosenAvatarIdx === i ? 'selected' : ''} ${avatarsLoading ? 'loading-slot' : ''}`}
                      onClick={() => !avatarsLoading && img && setChosenAvatarIdx(i)}
                    >
                      {avatarsLoading ? (
                        <div className="img-loading-shimmer" />
                      ) : img ? (
                        <>
                          <img src={img} alt={`Avatar ${i + 1}`} className="gen-image" />
                          {chosenAvatarIdx === i && <div className="img-check">✓</div>}
                        </>
                      ) : (
                        <div className="img-empty">Failed to generate</div>
                      )}
                    </div>
                    {!avatarsLoading && avatarLabels[i] && (
                      <p className="avatar-label">{avatarLabels[i]}</p>
                    )}
                  </div>
                ))}
              </div>
              ...
              {!avatarsLoading && (
                <div className="action-row">
                  <button
                    className="btn-primary"
                    disabled={chosenAvatarIdx === null}
                    onClick={handleGenerateScenes}
                  >
                    Lock Avatar & Build Scenes →
                  </button>
                  <button className="btn-secondary" onClick={handleGenerateAvatars}>
                    Regenerate Avatars
                  </button>
                </div>
              )}
          ...
          {step === STEPS.SCENES && (
            <div>
              <div className="step-heading">
                ...
                <p className="step-sub">
                  {scenesLoading ? `Generating ${sceneCount} scenes — 4 options each...` : 'Choose one image per scene to build your storyboard.'}
                </p>
              </div>
              ...
              {scenes.length > 0 && (
                <div className="scene-nav">
                  {scenes.map((scene, i) => (
                    <button
                      key={i}
                      className={`scene-tab ${currentScene === i ? 'active' : ''} ${scene.chosen !== null ? 'complete' : ''}`}
              ...
              {scenes[currentScene] && (
                <ImageGrid
                  images={scenes[currentScene].options}
                  selectedIdx={scenes[currentScene].chosen}
                  onSelect={(idx) => handleChooseSceneImage(currentScene, idx)}
                  loading={scenesLoading && (!scenes[currentScene]?.options?.length)}
                />
              )}

              {scenesLoading && !scenes[currentScene]?.options?.length && (
                <LoadingPulse message={`Generating scene ${currentScene + 1} of ${sceneCount}...`} />
              )}

              {/* Navigate between scenes / finish */}
              {!scenesLoading && scenes.length > 0 && (
                <div className="action-row">
                  {currentScene < scenes.length - 1 ? (
                    <button className="btn-primary" onClick={() => setCurrentScene(s => s + 1)}>
                      Next Scene →
                    </button>
                  ) : (
                    <button
                      className="btn-primary"
                      disabled={scenes.some(s => s.chosen === null)}
                      onClick={handleFinishStoryboard}
                    >
                      View Full Storyboard →
                    </button>
                  )}
                  {currentScene > 0 && (
                    <button className="btn-secondary" onClick={() => setCurrentScene(s => s - 1)}>
                      ← Previous Scene
                    </button>
                  )}
                </div>
          ...
          {step === STEPS.STORYBOARD && (
            <div>
              <div className="step-heading">
                ...
                <p className="step-sub">Your campaign is ready. All scenes at a glance.</p>
              </div>
                ...
                {scenes.map((scene, i) => {
                  ...
                  return (
                    <div key={i} className="storyboard-tile" onClick={() => { setCurrentScene(i); setStep(STEPS.SCENES) }}>
                      {imgUrl ? (
                        <img src={imgUrl} alt={`Scene ${i + 1}`} />
                      ) : (
                        <div style={{ aspectRatio: 1, background: '#111', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#333', fontSize: 12 }}>
                          No image
                        </div>
                      )}
              ...
              <div className="action-row">
                ...
                <button className="btn-secondary" onClick={() => setStep(STEPS.SCENES)}>
                  ← Back to Scenes
                </button>
              </div>
            </div>
